...
def detecteer_schakelcode(functions_resp):
    """Kies de aan/uit-code uit de functielijst: 'switch' > 'switch_1' > eerste 'switch*'."""
    codes = codes_from_functions(functions_resp)
    for pref in ("switch", "switch_1"):
        if pref in codes:
            return pref
    for c in codes:
...
def main():
    ...
    for t in toestellen:
        ...
        code = t.get("schakelcode") or detecteer_schakelcode(fn)
        info[t["naam"]] = {"id": t["id"], "code": code, "status": st}
        print(f"    {t['naam']}:")
        print(f"       status   : {st.get('result', st) if isinstance(st, dict) else st}")
        print(f"       functies : {codes_from_functions(fn) or fn}")

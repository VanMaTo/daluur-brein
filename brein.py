...
def fetch_prices(datum):
    ...
    Eerst REE (officieel, bereikbaar op de Pi, ENKEL de PVPC-reeks — nooit de
    spotprijs die 's middags naar 0 zakt); dan preciodelaluz als terugval
    (die is op sommige netwerken via DNS geblokkeerd, dus tweede keus)."""
    try:
        return _fetch_ree_pvpc(datum)
    except Exception as e:
        print(f"  REE-PVPC mislukt ({e}); probeer preciodelaluz...")

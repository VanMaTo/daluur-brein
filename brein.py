...
def main():
    ...
    # 5) schakelen — enkel wat niet klopt (onleesbaar telt als 'klopt niet' = fail-safe)
    #    UIT eerst in omgekeerde volgorde (WP vóór circulatie), dan AAN oplopend
    #    (circulatie vóór WP, zodat er flow is voor de WP aanslaat).
    beheerbaar = [t for t in te_beheren if info[t["naam"]]["code"]]
    uit = [t for t in beheerbaar if not doel[t["naam"]] and info[t["naam"]]["stand"] is not False]
    aan = [t for t in beheerbaar if doel[t["naam"]] and info[t["naam"]]["stand"] is not True]
    ...
    if not ops:
        print("  alle toestellen staan al goed — geen actie.")
    else:

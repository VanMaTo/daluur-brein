...
def get_standaard_n(groep):
    """Jouw blijvende standaard-aantal-uren voor deze groep (bv. minder filteruren
    in de winter). Los van de dagelijkse planning; valt enkel terug op DEFAULT_N
    als de tabel/rij ontbreekt of onbereikbaar is."""
    try:
        r = requests.get(f"{SUPABASE_URL}/rest/v1/instellingen?groep=eq.{groep}&select=n",
                         headers=_sb_headers(), timeout=15)
        r.raise_for_status()
        rows = r.json()
        if rows and isinstance(rows[0].get("n"), int) and 0 <= rows[0]["n"] <= 24:
            return rows[0]["n"]
    except Exception as e:
        print(f"    standaard ophalen ({groep}) mislukt: {e}")
    return DEFAULT_N
...
def bepaal_uren(groep, datum, prijzen):
    ...
    if prijzen:
        n = plan.get("n") if (plan and isinstance(plan.get("n"), int)) else get_standaard_n(groep)
